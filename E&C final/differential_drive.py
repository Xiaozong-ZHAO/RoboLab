...
def main():
    ...
    regulator.controllability_ayanlysis()
    # Define the cost matrices
    Qcoeff = np.array([310, 312, 200])
    Rcoeff = np.array([0.5, 0.1])
    Pcoeff = np.array([300, 400, 100])
    regulator.setTerminalCostMatrix(Pcoeff)
    ...
    # Plotting 
    # Plot the trajectory and theta of the robot
    base_pos_all = np.array(base_pos_all)
    base_bearing_all = np.array(base_bearing_all)
    # plot x against sample point and plot y against sample point in one figure
    plt.figure()
    ...
    # plt.figure()
    # plt.plot(base_pos_all[:, 0], base_pos_all[:, 1], label='Robot trajectory')
    # plt.scatter(0, 0, color='red', s=100, label='Destination')
    # plt.xlabel('x [m]')
    # plt.ylabel('y [m]')
    # plt.title('Robot trajectory')
    # plt.legend()
    ...
    # plt.show()
    # plt.figure()
    # plt.plot(base_bearing_all, label='Robot bearing')
    # plt.xlabel('Time [s]')
